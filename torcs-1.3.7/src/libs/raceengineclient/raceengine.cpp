	...
	if (car->_pit) {
		...
		if (car->_state & RM_CAR_STATE_PIT) {
			...
			if (car->_scheduledEventTime < s->currentTime) {
				...
				snprintf(buf, BUFSIZE, "%s pit stop %.1fs", car->_cname, info->totalPitTime);
				ReRaceMsgSet(buf, 5);
			} else {
				   ...
				   (s->_maxDammage == 0 || car->_dammage <= s->_maxDammage))
		{
			...
			if ((lgFromStart > car->_pit->lmin) && (lgFromStart < car->_pit->lmax)) {
				...
				if (pitok) {
					...
					snprintf(buf, BUFSIZE, "%s in pits", car->_cname);
					ReRaceMsgSet(buf, 5);
				...
				if ((car->_state & RM_CAR_STATE_FINISH) == 0) {
					...
					if ((car->_remainingLaps < 0) || (s->_raceState == RM_RACE_FINISHING)) {
						...
						if (ReInfo->s->_raceType == RM_TYPE_RACE) {
							if (car->_pos == 1) {
								snprintf(buf, BUFSIZE, "Winner %s", car->_cname);
								...
								snprintf(buf, BUFSIZE, "%s Finished %d%s", car->_cname, car->_pos, numSuffix);
								ReRaceMsgSet(buf, 5);
							}
						}
					}
...
ReRaceRules(tCarElt *car)
{
	...
	if (prevSeg->raceInfo & TR_PITSTART) {
		...
		if (seg->raceInfo & TR_PIT) {
			...
			if (penalty) {
				switch (penalty->penalty) {
					case RM_PENALTY_DRIVETHROUGH:
						snprintf(buf, BUFSIZE, "%s DRIVE THROUGH PENALTY CLEANING", car->_cname);
						ReRaceMsgSet(buf, 5);
						rules->ruleState |= RM_PNST_DRIVETHROUGH;
						break;
					case RM_PENALTY_STOPANDGO:
						snprintf(buf, BUFSIZE, "%s STOP&GO PENALTY CLEANING", car->_cname);
						ReRaceMsgSet(buf, 5);
		...
		if (seg->raceInfo & TR_PIT) {
			...
			if (rules->ruleState & (RM_PNST_DRIVETHROUGH | RM_PNST_STOPANDGO_OK)) {
				/* clear the penalty */
				snprintf(buf, BUFSIZE, "%s penalty cleared", car->_cname);
				ReRaceMsgSet(buf, 5);
				penalty = GF_TAILQ_FIRST(&(car->_penaltyList));
			...
			if (!(rules->ruleState & RM_PNST_STNGO)) {
				snprintf(buf, BUFSIZE, "%s STOP&GO PENALTY", car->_cname);
				ReRaceMsgSet(buf, 5);
	...
	if (seg->raceInfo & TR_SPEEDLIMIT) {
		if (!(rules->ruleState & (RM_PNST_SPD | RM_PNST_STNGO)) && (car->_speed_x > track->pits.speedLimit)) {
			snprintf(buf, BUFSIZE, "%s DRIVE THROUGH PENALTY", car->_cname);
			ReRaceMsgSet(buf, 5);
