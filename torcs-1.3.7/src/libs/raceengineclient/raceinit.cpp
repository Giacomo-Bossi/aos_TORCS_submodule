		...
		for (j = 0; j < MAX_MOD_ITF; j++) {
			if ((*(ReInfo->modList))->modInfo[j].index == robotIdx) {
				...
				if (robhdle != NULL) {
					...
					strncpy(elt->_cname, GfParmGetStr(robhdle, path, ROB_ATTR_NAME, "<none>"), MAX_NAME_LEN - 1);
					elt->_cname[MAX_NAME_LEN - 1] = 0;
