	...
	switch (ReInfo->s->_raceType) {
		case RM_TYPE_RACE:
			...
			for (i = 0; i < s->_ncars; i++) {
				...
				GfParmSetStr(results, path, RE_ATTR_NAME, car->_cname);
		...
		case RM_TYPE_PRACTICE:
			car = s->cars[0];
			snprintf(path, BUFSIZE, "%s/%s/%s", ReInfo->track->name, RE_SECT_RESULTS, race);
			GfParmSetStr(results, path, RM_ATTR_DRVNAME, car->_cname);
			break;
...
ReUpdateQualifCurRes(tCarElt *car)
{
	...
	snprintf(buf, BUFSIZE, "%s on %s - Lap %d", car->_cname, ReInfo->track->name, car->_laps);
	...
	for (i = 1; i < nCars; i++) {
		...
		if (!printed) {
			if ((car->_bestLapTime != 0.0) && (car->_bestLapTime < GfParmGetNum(results, path, RE_ATTR_BEST_LAP_TIME, NULL, 0))) {
				...
				snprintf(buf, BUFSIZE, "%d - %s - %s (%s)", i, timefmt, car->_cname, carName);
