...
cGrBoard::loadDefaults(tCarElt *curCar)
{
	...
	if (curCar->_driverType == RM_DRV_HUMAN) {
		snprintf(path, BUFSIZE, "%s/%s", GR_SCT_DISPMODE, curCar->_cname);
...
cGrBoard::grDispCarBoard1(tCarElt *car, tSituation *s)
{
	...
	y = Winy + Winh - dy - 5;
	snprintf(buf, BUFSIZE, "%d/%d - %s", car->_pos, s->_ncars, car->_cname);
...
cGrBoard::grDispCarBoard2(tCarElt *car, tSituation *s)
{
	...
	if (car->_pos != 1) {
		snprintf(buf, BUFSIZE, "<- %s", s->cars[car->_pos - 2]->_cname);
	...
	if (car->_pos != s->_ncars) {
		snprintf(buf, BUFSIZE, "-> %s", s->cars[car->_pos]->_cname);
		GfuiPrintString(buf, clr, GFUI_FONT_SMALL_C, x, y, GFUI_ALIGN_HL_VB);
...
cGrBoard::grDispLeaderBoard(tCarElt *car, tSituation *s)
{
	...
	for (j = maxi; j > 0; j--) {
		...
		snprintf(buf, BUFSIZE, "%3d: %s", i, s->cars[i-1]->_cname);
		GfuiPrintString(buf, clr, GFUI_FONT_SMALL_C, x, y, GFUI_ALIGN_HL_VB);
...
#define YM	10
	...
	x = Winx + Winw / 2;
	snprintf(buf, BUFSIZE, "%s", car->_cname);
