...
void cGrScreen::selectTrackMap()
{
	...
	if (curCar->_driverType == RM_DRV_HUMAN) {
		snprintf(path2, BUFSIZE, "%s/%s", GR_SCT_DISPMODE, curCar->_cname);
...
void cGrScreen::selectCamera(long cam)
{
	...
	snprintf(path, BUFSIZE, "%s/%d", GR_SCT_DISPMODE, id);
	GfParmSetStr(grHandle, path, GR_ATT_CUR_DRV, curCar->_cname);
...
void cGrScreen::update(tSituation *s, float Fps)
{
	...
	if (carChanged) {
		...
		GfParmSetStr(grHandle, buf, GR_ATT_CUR_DRV, curCar->_cname);
		loadParams (s);
		GfParmWriteFile(NULL, grHandle, "Graph");
...
void cGrScreen::loadParams(tSituation *s)
{
	...
	if (!curCar) {
		...
		for (i = 0; i < s->_ncars; i++) {
			if (!strcmp(s->cars[i]->_cname, carName)) {
				break;
			}
		}
		if (i < s->_ncars) {
